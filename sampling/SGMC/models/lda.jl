...
function lda( train::Array{ Int64, 2 }, test::Array{ Int64, 2 }, K::Int )
    ...
    return( lda( M, V, K, theta, train, test, zcounts, beta, alpha) )
end
...
"""
function grad_theta_prior( model::lda )
    dtheta = zeros( model.K, model.V )
    for ( k in 1:model.K )
        for ( v in 1:model.V )
            dtheta[k,v] += model.beta - exp( model.theta[k,v] )
        end
    end
    return dtheta
end
...
"""
function grad_theta( model::lda, m_subsample::Array{ Int64, 1 } )
    dtheta = zeros( model.K, model.V )
...
function update_topics( model::lda, m_subsample::Array{ Int64, 1 } )
    ...
    exp_theta = exp( model.theta )
    ndz = slice( sum( model.zcounts, 3 ), (:,:,1) )
    prob_current = zeros( model.K )
    for ( i in 1:length( m_subsample ) )
        m = m_subsample[i]
        zcounts_current = findnz( slice( model.zcounts, (m,:,:) ) )
        ...
        for ( j in 1:num_nonzeros )
            ...
            prob_current ./= denom
            zcounts_update[i,:,v] += rand( Multinomial( n_words, prob_current ), 1 )'
        end
    end
...
function perplexity( model::lda, test::Array{ Int64, 2 } )
    ...
    doc_topic_est = ( sum( model.zcounts[:,:,:], 3 ) + 1 ) 
    doc_topic_est ./= ( sum( model.zcounts[:,:,:], (2,3) ) + model.K )
    for ( m in 1:model.M )
        for ( v in 1:model.V )
